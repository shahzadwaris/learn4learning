...
$auth=Auth::User()->id;
 $teacherhomeworkdetail=DB::table('homework')
                               ...
                               ->join('users', 'users.id', '=', 'homework.teacher_id')
                                ...
                                ->select('subjects.id as subject_id', 'subjects.name as subjectname',
                                          'lessons.id as lessons_id','lessons.title as lessons_title', 'lessons.description as lessons_des',
                                           'homework.*', 'users.fname', 'homework.date as Duedate' , 'lessons.thumbnail as lesson_thum',  'lessons.document as lessonsDoc')
                                ->limit(10)
                                ->get();
                            ...
                            $favsubject=DB::table('subject_level_details')->where('subject_level_details.user_id', $auth)
...
$experices=DB::table('experiences')->where('experiences.teacher_id', $auth)->select('experiences.*')->limit(5)->get();


// foreach ($Book as $Booing) {
//   $bookings[]=$Booing->lessonsid;
// }
// print_r($bookings);
// exit();
$uid=Auth::user()->id;
...
// dd(auth::user());
        return view('frontend.pages.teachers.teacher-homepage')->with(['Book'=>$Book,
         'teacherhomeworkdetail'=>$teacherhomeworkdetail, 'usersimgg'=>$usersimgg, 'sepbooking'=>$sepbooking, 'experices'=>$experices, 'favsubject'=>$favsubject]);
    }
...
//             $schedules[$key]['teacher_thumbnail']=User::where('id',$schedule['user_id'])->pluck("thumbnail")->first();
    ...
    public function SearchSchedule(Request $request){
      ...
      if (!empty($request->date_id)) {
       ...
       $Book = $Book->select('users.*', 'lessons.id as lessonsid', 'lessons.*', 'lessons.date as Lesson_date',
       'lessons.time as Lesson_time', 'users.thumbnail as USerthumbnail' , 
  ...
  return view('auth.teachers.teacher-subjects',compact('subjects','user_id'));
    ...
    public function MySubStudents(){
        ...
        $getmystydentrecord=DB::table('student_lessons')
                                     ...
                                     ->select('users.*', 'student_lessons.id as student_lessons_id', 'lessons.id as lesson_id', 'levels.name as level_name', 'users.id as user_id',
    ...
    public function MYSubjects(){
        ...
        $getmySyb=DB::table('student_lessons')
                                     ...
                                     ->select('student_lessons.id as student_lessons_id', 'lessons.id as lesson_id', 'levels.name as level_name', 'users.id as user_id',
                                      'subjects.name as Subject_name')
                                     ->OrderBy('users.id','DESC')
                                     ->get();
    ...
    public function View_student_profile($id)
    {
      ...
      $getrecordindividulStuRecord=DB::table('users')
                                     ...
                                     ->select('student_lessons.id as student_lessons_id','levels.name as level_name', 'users.id as user_id',
                                      'subjects.name as Subject_name', 'lessons.description', 'lessons.title' , 'lessons.date' , 'users.*')
                                     ->OrderBy('users.id','DESC')
                                     ->get();
